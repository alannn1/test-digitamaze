            ...
            <tbody>
                @foreach ($guru as $gurus)
                    <tr class="bg-white border-b bg-gray-500 border-gray-700">
                        <th scope="row" class="px-6 py-4">{{ $gurus->nama }}</th>
                        <td class="px-6 py-4">{{ $gurus->nip }}</td>
                        <td class="px-6 py-4">{{ $gurus->kelas->nama_kelas ?? '-' }}</td>
                        <td>
                            <button type="button" wire:click="edit({{ $gurus->id }})" class="px-2 py-1 bg-blue-500 rounded-md">Edit</button>
                            <button type="button" wire:click="delete({{ $gurus->id }})" class="px-2 py-1 bg-blue-500 rounded-md">Delete</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
